...
const AdminDepartmentCards = ({
  ...
  return (
    <section style={{ ...getSectionBg(), padding: '72px 48px' }}>
      {/* Header */}
      ...
      {/* Row 1 — tối đa 3 card */}
      <div style={{ display: 'flex', gap: 28, justifyContent: 'center', marginBottom: 28, flexWrap: 'wrap' }}>
        {firstRow.map((card, i) => <CardItem key={i} card={card} />)}
      </div>

      {/* Row 2 — phần còn lại */}
      {secondRow.length > 0 && (
        <div style={{ display: 'flex', gap: 28, justifyContent: 'center', flexWrap: 'wrap' }}>
          {secondRow.map((card, i) => <CardItem key={i + 3} card={card} />)}
        </div>
      )}
    </section>
  );
