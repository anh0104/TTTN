...
export default function AboutPage({ text1='Với tinh thần "Chạm mê tít – Snap into Joy", metik mong muốn trở thành người bạn đồng hành.', text2='metik không chỉ là một sản phẩm snack. metik là cảm giác giòn vui khi mở gói.', videoUrl='', videoThumb='' }={}) {
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:56, alignItems:'center' }}>
          <div style={{ borderRadius:12,overflow:'hidden',background:'#111',aspectRatio:'16/9',position:'relative',cursor:'pointer' }} onClick={()=>setPlaying(true)}>
            {!playing ? <>
              ...
              <div style={{ position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:videoThumb?'rgba(0,0,0,0.25)':'rgba(0,0,0,0.5)' }}><div style={{ width:56,height:56,borderRadius:'50%',background:'rgba(255,255,255,0.9)',display:'flex',alignItems:'center',justifyContent:'center' }}><span style={{ fontSize:20,marginLeft:4 }}>▶</span></div></div>
              <div style={{ position:'absolute',bottom:0,left:0,right:0,background:'rgba(0,0,0,0.8)',padding:'5px 10px',display:'flex',alignItems:'center',gap:8 }}><span style={{ color:'#fff',fontSize:11 }}>▶ 00:00</span><div style={{ flex:1,height:3,background:'#555',borderRadius:2 }}/><span style={{ color:'#fff',fontSize:11 }}>00:18 🔊</span></div>
