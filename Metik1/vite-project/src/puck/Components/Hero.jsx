...
export default function Hero({ slides } = {}) {
  const list = Array.isArray(slides) && slides.length > 0 ? slides : [{ image: '', fallbackBg: 'linear-gradient(135deg,#87CEEB,#E8F4F8)' }]
  ...
  const slide = list[cur] || list[0]
  return (
    <section style={{ position:'relative', width:'100%', overflow:'hidden' }}>
      <div style={{ position:'relative', minHeight:420, background: slide.fallbackBg || 'linear-gradient(135deg,#87CEEB,#E8F4F8)' }}>
        {slide.image && <img src={slide.image} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} onError={e => e.target.style.display='none'} />}
      ...
      {list.length > 1 && <>
        <button onClick={() => setCur(c => (c-1+list.length)%list.length)} style={{ position:'absolute',left:12,top:'50%',transform:'translateY(-50%)',width:36,height:36,borderRadius:'50%',background:'rgba(255,255,255,0.8)',border:'none',cursor:'pointer',fontSize:18 }}>‹</button>
        <button onClick={() => setCur(c => (c+1)%list.length)} style={{ position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',width:36,height:36,borderRadius:'50%',background:'rgba(255,255,255,0.8)',border:'none',cursor:'pointer',fontSize:18 }}>›</button>
      </>}
