    ...
    <section style={{ background:'#fff', padding:'32px 0 64px' }}>
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 32px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:24 }}>
          {sp.map((p,i)=>(
            <a key={i} href={`/san-pham/${p.slug||'sp'}`} style={{ textDecoration:'none' }}>
              <div style={{ background:'#fff',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,0.08)',border:'1px solid #eee',transition:'transform 0.2s' }} onMouseEnter={e=>e.currentTarget.style.transform='translateY(-4px)'} onMouseLeave={e=>e.currentTarget.style.transform='none'}>
                <div style={{ height:260, overflow:'hidden', background:'#f8f8f8' }}><img src={p.image||FB[i%4]} alt={p.name||''} style={{ width:'100%',height:'100%',objectFit:'cover',display:'block' }} onError={e=>e.target.src=FB[i%4]}/></div>
                <div style={{ padding:'14px 16px 18px', textAlign:'center' }}><span style={{ color:'#E07820',fontWeight:700,fontSize:15 }}>{p.name||''}</span></div>
              </div>
            </a>
          ))}
        </div>
